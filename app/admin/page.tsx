...
export default async function AdminPage() {
  ...
  const { data: posts, error } = await supabase.from("posts").select("*").order("updated_at", { ascending: false });
  return (
    <main className="admin-page">
      {error ? <section className="admin-auth-card"><h1>Không đọc được dữ liệu</h1><p>{error.message}</p></section> : <AdminDashboard initialPosts={(posts ?? []) as AdminPost[]} userId={user.id} userEmail={user.email ?? ""} />}
    </main>
  );
}
